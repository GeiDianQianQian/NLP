...
mat_test = np.zeros(shape=(51135, 27))

mat_train = np.zeros(shape=(25568, 28))

header_test = ['f0', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'f9',
          'f10', 'f11', 'f12', 'f13', 'f14', 'f15', 'f16', 'f17', 'f18', 'f19',
          'f20', 'f21', 'f22', 'f23', 'f24', 'f25', 'f26']

header_train = ['f0', 'f1', 'f2', 'f3', 'f4', 'f5', 'f6', 'f7', 'f8', 'f9',
          'f10', 'f11', 'f12', 'f13', 'f14', 'f15', 'f16', 'f17', 'f18', 'f19',
...
for n, (h1, h2, e) in enumerate(islice(sentences(), opts.num_sentences)):
    ...
    vc1.extend(sw1)
    vc2.extend(sw2)
    diff_test = list(np.subtract(vc1, vc2))
    mat_test[n]=diff_test
    if n < 25568:
        diff_train = diff_test
        diff_train.append(truth[n])
        mat_train[n] = diff_train
    '''
    l= 0.0
...
with open('feat_train.csv', 'wb') as f:
    writer_train = csv.writer(f)
    writer_train.writerow(header_train)
    writer_train.writerows(mat_train)

with open('feat_test.csv', 'wb') as f:
    writer_test = csv.writer(f)
    writer_test.writerow(header_test)
    writer_test.writerows(mat_test)